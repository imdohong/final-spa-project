...
const generateAIReview = async () => {
  ...
  try {
    const title = store.selectedMovie?.title || '제목 정보 없음';
    const genres = store.selectedMovie?.genres?.map(g => g.name).join(', ') || '장르 정보 없음';
    const rating = store.selectedMovie?.vote_average
      ? store.selectedMovie.vote_average.toFixed(1)
      : '평점 정보 없음';
...
onMounted(async () => {
  if (movieId.value) {
    await store.fetchMovieDetail(movieId.value);
  }
});

watch(movieId, async (newId) => {
  if (newId) {
    await store.fetchMovieDetail(newId);
  }
});
...
<template>
  <main v-if="store.selectedMovie" class="detail-page">
    <div
      ...
      :style="{
        backgroundImage: store.selectedMovie.backdrop_path
          ? `url(https://image.tmdb.org/t/p/original${store.selectedMovie.backdrop_path})`
          : 'none'
      }"
    >
        ...
        <div class="info-zone">
          ...
          <div class="meta-dashboard">
            <span class="badge rating">
              ⭐ {{ store.selectedMovie?.vote_average ? store.selectedMovie.vote_average.toFixed(1) : '0.0' }} / 10
            </span>
            <span class="badge runtime">
              🕘 {{ store.selectedMovie?.runtime || '정보 없음' }}분
            </span>
            <span class="badge release">
              🗓️ {{ store.selectedMovie?.release_date || '정보 없음' }} 개봉
            </span>
          </div>
  ...
  <div v-else class="full-screen-loading-gate">
    <div class="loading-spinner"></div>
    <p class="loading-text">
      영화 상세 정보를 불러오는 중입니다...
    </p>
